-- Execute this SQL to apply the roommate booking enhancements
-- Run this in your MySQL database

USE hostel_db; -- Using the hostel_db database name

-- Table for storing roommate room agreements
CREATE TABLE IF NOT EXISTS `roommate_room_agreements` (
...
SELECT 
    ...
    rd.seater,
    rd.fees,
    COALESCE(rm.match_score, 0) as compatibility_score,
    COALESCE(sp1.branch, 'Not specified') as student1_branch,
    COALESCE(sp2.branch, 'Not specified') as student2_branch,
    COALESCE(b1.branch_name, 'Not specified') as student1_branch_name,
    COALESCE(b2.branch_name, 'Not specified') as student2_branch_name
...
LEFT JOIN roommate_matches rm ON rra.roommate_pair_id = rm.id
...
SELECT 'Roommate booking enhancement tables created successfully!' as Status;